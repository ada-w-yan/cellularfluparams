...
deriv(V_inf) <- max(0, p_inf * sum(I_scale_vec)) - max(0, c_inf * V_inf) - max(0, beta_inf * T1 * V_inf)
deriv(V_tot) <- max(0, p_tot * sum(I_scale_vec)) - max(0, c_tot * V_tot) - max(0, beta_tot * T1 * V_inf)
